...
class load_KGs_data(object):
    def __init__(self, myconfig):
        ...
        with open(myconfig.datasetPath + 'kgs_num', 'r') as ff:
        ...
        ## Attribute triples
        if 'M' in myconfig.model_type:
            self.attr_triples_id = fileUtil.load_triples_id(myconfig.datasetPath + 'attr_triples_id')
            KGs_attr_triple = np.array(self.attr_triples_id)
            KGs_attr_triple = KGs_attr_triple.astype(np.int)

            # attribute value embedding
            kg_value_embed = fileUtil.loadpickle(myconfig.datasetPath + 'value_embedding.out')
