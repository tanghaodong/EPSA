...
class HET_attr_align2(nn.Module):
    ...
    def forward(self):
        ...
        # 2
        m_embed_1 = self.add_m_layer(se_layer, value_embed)  # (M,dim)
        #m_embed_1 = self.primal_m_0 + self.alpha * m_embed_1
        ce_embed = self.add_ce_att_layer(se_layer, m_embed_1, value_embed)   # (E,dim)
...
#############################################
    def get_loss(self, es_embed, ec_embed, path_embed, e_out_embeds, tt_neg_pairs_es, tt_neg_pairs_ec, tt_neg_pairs_path, tt_neg_pairs_e_out):
        loss1 = self.get_loss_each(es_embed, tt_neg_pairs_es)
        loss2 = self.get_loss_each(ec_embed, tt_neg_pairs_ec)
        loss3 = self.get_loss_each(path_embed, tt_neg_pairs_path)
        loss4 = self.get_loss_each(e_out_embeds, tt_neg_pairs_e_out)

        output_layer = torch.cat((es_embed, ec_embed, path_embed, e_out_embeds), -1)
        return loss1 + self.l_bata * loss2 + self.l_bata * loss3 + self.l_bata * loss4, output_layer
